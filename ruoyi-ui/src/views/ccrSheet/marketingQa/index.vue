<template>
  <div class="app-container">
    ...
    <div class="page-main">
      ...
      <el-row :gutter="10" class="mb8 cssBuntton">
        ...
        <el-badge class="itemBadge" v-for="(item,index) of statusSearchList" :key="index">
          <el-button @click="handleCheckedChange(item.value)" :type="queryParams.queryType== item.value ? 'primary' : 'info'">
            ...
            <span v-if="item.value == 3">{{operationDoneTotal}}</span>
            <span v-if="item.value == 4">{{unEvaluateTotal}}</span>
          </el-button>
        </el-badge>
      ...
      <div class="content">
        <el-table stripe ref="table" v-loading="loading" :data="list">
          ...
          <el-table-column align="center" label="提报人满意度" prop="customerSatisfaction" min-width="160px"/>
    ...
    <add v-if="visible" ref="detail" :ccrId="ccrId" title="工单详情" submitUrl="marketDeptSubmit" :pagetype="1"
         @emitInit="getList"/>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        statusSearchList: [
          { label: 'All', value: 1 },
          { label: '未处理', value: 2 },
          { label: '运营质量已完成', value: 3 },
          { label: '提报人未评价', value: 4 }
        ],
        noTreatedTotal: '',   // 未处理总数
        operationDoneTotal: '',  // 运营质量已完成总数
        unEvaluateTotal: '',    // 提报人未评价总数
        statusList: [
          { label: '未处理', value: 1 },
          { label: '运营质量评估中', value: 2 },
          { label: '未处理', value: 3 },
          { label: '运营质量评估中', value: 4 },
          { label: '未处理', value: 5 },
          { label: '运营质量评估中', value: 6 },
          { label: '未处理', value: 7 },
          { label: '运营质量评估中', value: 8 },
          { label: '未处理', value: 9 },
          { label: '运营质量评估中', value: 10 },
          { label: '未处理', value: 11 },
          { label: '运营质量评估中', value: 12 },
          { label: '未处理', value: 13 },
          { label: '运营质量已完成', value: 14 },
          { label: '客户未评价', value: 15 },
          { label: '已完成', value: 16 }
        ],
        ccrId: ''
      }
    ...
    methods: {
      ...
      getList(type) {
        ...
        this.loading = true
        ccrmarketFeedbackListApi({ ...this.queryParams }).then(res => {
          if (res.code == 200) {
      ...
      noGetList() {
        ccrgetFeedBackCountApi({
          ...
          if (res.code == 200) {
            ...
            this.operationDoneTotal = res.data.operationDoneTotal
            this.unEvaluateTotal = res.data.unEvaluateTotal
          }
        })
      },
