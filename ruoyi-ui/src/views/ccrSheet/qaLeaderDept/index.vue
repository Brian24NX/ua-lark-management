<template>
  <div class="app-container">
      ...
      <el-row :gutter="10" class="mb8 cssBuntton">
        ...
        <el-badge class="itemBadge" v-for="(item,index) of statusSearchList" :key="index">
          <el-button @click="handleCheckedChange(item.value)"
            ...
            {{item.label}}
            <span v-if="item.value == 2">{{urgeTotal}}</span>
    ...
    <add v-if="visible" ref="detail" :ccrId="ccrId" title="工单详情" submitUrl="presidentDeptSubmit" :pagetype="5" @emitInit="getList"/>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      getList(type) {
        ...
        this.loading = true
        ccrpresidentFeedbackListApi({ ...this.queryParams }).then(res => {
          if (res.code == 200) {
      ...
      noGetList() {
        ccrgetFeedBackCountApi({
          ...
          if (res.code == 200) {
            this.urgeTotal = res.data.urgeTotal
