<template>
  <div class="app-container">
    ...
    <div class="page-main">
      ...
      <el-row :gutter="10" class="mb8 cssBuntton">
        ...
        <el-badge class="itemBadge" v-for="(item,index) of statusSearchList" :key="index">
          <el-button @click="handleCheckedChange(item.value)"
                     :type="queryParams.queryType== item.value ? 'primary' : 'info'">
            {{item.label}}
            <span v-if="item.value == 2">{{noTreatedTotal}}</span>
            <span v-if="item.value == 3">{{noAssortedTotal}}</span>
            <span v-if="item.value == 4">{{noMeasuresTotal}}</span>
            <span v-if="item.value == 5">{{noCostAttributionTotal}}</span>
          </el-button>
        </el-badge>
      ...
      <div class="content">
        <el-table stripe ref="table" v-loading="loading" :data="list">
          ...
          <el-table-column align="center" label="状态" prop="status" min-width="160px" :formatter="formatStatus"/>
    ...
    <add v-if="visible" ref="detail" :ccrId="ccrId" title="工单详情" submitUrl="operationDeptSubmit" :pagetype="2"
         @emitInit="getList"/>
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 查询类型（1:All 2:未处理 3:问题待分类 4:措施未提交 5:待确认费用归属）
        statusSearchList: [
          { label: 'All', value: 1 },
          { label: '未处理', value: 2 },
          { label: '问题待分类', value: 3 },
          { label: '措施未提交', value: 4 },
          { label: '待确认费用归属', value: 5 }
        ],

        noTreatedTotal: '',   // 未处理总数
        noAssortedTotal: '',  // 问题待分类总数
        noMeasuresTotal: '',    // 措施未提交总数
        noCostAttributionTotal: '',    // 待确认费用归属总数
        ...
        statusList: [
          ...
          { label: '问题待分类', value: 8 },
          { label: '措施未提交', value: 9 },
          { label: '措施未提交', value: 10 },
          { label: '待确认费用归属', value: 11 },
          { label: '待确认费用归属', value: 12 },
          { label: '已完成', value: 13 },
          { label: '已完成', value: 14 },
          { label: '已完成', value: 15 },
          { label: '已完成', value: 16 }
    ...
    methods: {
      formatStatus(row, column, cellValue, index) {
        if (row.status) {
          let arr = this.statusList.filter((item) => item.value === row.status)
      ...
      getList(type) {
        ...
        this.loading = true
        ccroperationFeedbackListApi({ ...this.queryParams }).then(res => {
          if (res.code == 200) {
      ...
      noGetList() {
        ccrgetFeedBackCountApi({
          ...
          if (res.code == 200) {
            ...
            this.noAssortedTotal = res.data.noAssortedTotal
            this.noMeasuresTotal = res.data.noMeasuresTotal
            this.noCostAttributionTotal = res.data.noCostAttributionTotal
          }
        })
      },
