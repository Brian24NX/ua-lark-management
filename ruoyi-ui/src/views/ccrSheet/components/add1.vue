  ...
  <el-dialog class="ccrSheet-dialog" title="title" :visible.sync="visible" width="1200px"
    ...
    <div class="el-dialog__title" slot="title">
      ...
      <span class="status-default"
            :class="[form.statusName==='未处理'?'status-no':'', form.statusName==='已完成'?'status-success':'']">{{form.statusName}}</span>
    </div>

    <el-form v-loading="loading" :inline="true" :disabled="form.status==16" :model="form" :rules="rules" ref="form">
...
<!--                            v-model.trim="form.additionInfo" placeholder="" clearable/>-->
            ...
            <el-row class="mt20">
              <el-col :span="24">
                ...
                <div class="css-value">
                  <bigMedium ref="additionResourceRef" :key="bigIndex" v-model="form.additionResource" :isFile="pagetype===1 && [1,3,5,7,9,11,13].indexOf(form.status) !== -1"></bigMedium>
<!--                  <bigMedium ref="additionResourceRef" :key="bigIndex" v-model="form.additionResource" :isFile="subFlag"></bigMedium>-->
                </div>
              </el-col>
            </el-row>
          </div>
          <div class="template-bg" v-if="!(form.status == 16 || (pagetype==1 && [1,3,5,7,9,11,13].indexOf(form.status) !== -1))"></div>
<!--          <div class="template-bg" v-if="!subFlag"></div>-->
            ...
            <el-row class="mt20">
              <el-col :span="24">
                <el-form-item label="应急方案：" prop="emergencyPlan"
                  ...
                  <el-input :disabled="!(pagetype===1 && [1,3,5,7,9,11,13].indexOf(form.status) !== -1)"
                            type="textarea" style="min-width: 400px;" :rows="5" v-model.trim="form.emergencyPlan"/>
                </el-form-item>
              </el-col>
            </el-row>
          </div>
        ...
        <div class="template">
          <div class="template-contant">
            ...
            <el-row v-if="form.isQualityProblem == 1" class="mt20">
              <el-col :span="7">
                <el-form-item label="退回单号：" prop="returnNumber"
                              :rules="pagetype===3 && [5,6].indexOf(form.status) !== -1 ? { required: true, message: '此项为必填项', trigger: ['blur','change']} : {}">
                  <el-input :disabled="!(pagetype===3 && [5,6].indexOf(form.status) !== -1)" v-model.trim="form.returnNumber"
                            type="text"/>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="预计送达时间：" prop="expectedDeliveryTime"
                              :rules="pagetype===3 && [5,6].indexOf(form.status) !== -1 ? { required: true, message: '此项为必填项', trigger: ['blur','change']} : [{}]">
                  <el-date-picker
                    :disabled="!(pagetype===3 && [5,6].indexOf(form.status) !== -1)"
          ...
          <div class="template-bg"
               v-if="!(form.status == 16 || (pagetype==2 && [1,2].indexOf(form.status) !== -1) || pagetype==3 && [5,6].indexOf(form.status) !== -1)"></div>
        </div>
        <el-divider></el-divider>
            ...
            <el-row>
              ...
              <el-col :span="12">
                ...
                <el-button
                  ...
                  @click="rejectionCountFn(form.rejectionCount)" type="text" class="danger">不认可，请重新分配
                </el-button>
              </el-col>
            </el-row>
            ...
            <el-row class="mt20">
              <el-col :span="24">
                <el-form-item label="报告：" prop="report" label-width="120px" label-position="left"
                  ...
                  <!--                    -->
                  <input ref="reportFile" type="file" name="" style="display: none" @change="onChange"/>
        ...
        <div class="template">
          ...
          <div class="template-bg"
               v-if="!(form.status == 16 || (pagetype==1 && [14].indexOf(form.status) !== -1))"></div>
        </div>
        <el-divider></el-divider>
  ...
  export default {
    ...
    methods: {
        ...
        if (this.ccrId) {
          ...
          ccrqueryFeedbackDetailsApi(this.ccrId).then(res => {
            if (res.code == 200) {
              ...
              this.subFlag =
                (pagetype==1 && [2,4,6,8,10,12,15,16].indexOf(status) === -1) ||
      ...
      onChange() {
        ...
        if (['doc', 'docx', 'xlsx', 'xls', 'pdf', 'ppt','PPT'].indexOf(ext) === -1) {
          this.$message.error('请上传格式为 doc,docx,xlsx,xls,pdf 的文件')
          return
        }
