<template>
  <div class="app-container">
    ...
    <add v-if="visible" ref="detail" :ccrId="ccrId" title="工单详情" submitUrl="responseDeptSubmit" :pagetype="4"
         @emitInit="getList"/>
  </div>
</template>
...
<script>
  ...
  import {
    ...
    ccrresponseDeptFeedbackListApi,
  ...
  export default {
    ...
    methods: {
      ...
      getOperatorIds() {
        let roles = this.$store.state.user.userData.roles
        let idArr = []
        if (this.$store.state.user.userData.deptId) {
          idArr.push(this.$store.state.user.userData.deptId)
        }
        if (roles || roles.length > 0) {
          roles.forEach((item) => {
            if (!!item.deptIds && item.deptIds.length > 0) {
              idArr.push(...item.deptIds)
            }
          })
        }
        return idArr
      ...
      getList(type) {
        ...
        this.loading = true
        this.queryParams.operatorId = this.getOperatorIds()
        ccrresponseDeptFeedbackListApi({ ...this.queryParams }).then(res => {
          if (res.code == 200) {
      ...
      noGetList() {
        ccrgetFeedBackCountApi({
          deptType: 4,
          operatorId: this.getOperatorIds()
        }).then(res => {
          if (res.code == 200) {
