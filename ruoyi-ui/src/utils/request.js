  ...
  if (!isRepeatSubmit && (config.method === 'post' || config.method === 'put')) {
    ...
    if (sessionObj === undefined || sessionObj === null || sessionObj === '') {
      ...
      if (s_data === requestObj.data && requestObj.time - s_time < interval && s_url === requestObj.url) {
        const message = i18n.t('dataBeing')
...
service.interceptors.response.use(res => {
    ...
    if (code === 401) {
      if (!isRelogin.show) {
        isRelogin.show = true
        MessageBox.confirm(i18n.t('reLoginMessage'), i18n.t('systemPrompt'), {
      ...
      Cookies.remove('oss-Token')
      return Promise.reject(i18n.t('noToken'))
    } else if (code === 500) {
      Message({
        message: msg,
        type: 'error'
  ...
  error => {
    ...
    if (message == 'Network Error') {
      ...
      message = i18n.t('networkErrorTimeout')
    } else if (message.includes('Request failed with status code')) {
      message = i18n.t('systemInterface') + message.substr(message.length - 3) + i18n.t('abnormal')
    }
    Message({
      message: message,
      type: 'error',
...
export function download(url, params, filename, config) {
  ...
  return service.post(url, params, {
    ...
    console.error(r)
    Message.error(i18n.t('errorDownloadingFile'))
    downloadLoadingInstance.close()
  })
}
